...
/* USER CODE BEGIN 0 */
//ADC电流�????????????????????????测引脚分配如下：ADC1->OPAMP1->c_in; ADC2->OPAMP2->c_cap; ADC3->OPAMP3->c_chassis
//ADC电压�????????????????????????测测引脚分配如下：ADC4->OPAMP6->v_in; ADC5->OPAMP5->v_cap
void ADC_Calib_Start(void)
{
    ...
    HAL_ADC_Start_DMA(&hadc1, Fmac_Wdata, buf_len);
//    HAL_ADC_Start_DMA(&hadc1, (uint32_t *) &c_in.value_origin, buf_len);
    HAL_ADC_Start_DMA(&hadc2, (uint32_t *) &c_cap.value_origin, buf_len);
    HAL_ADC_Start_DMA(&hadc3, (uint32_t *) &c_chassis.value_origin, buf_len);
    HAL_ADC_Start_DMA(&hadc4, (uint32_t *) &v_in.value_origin, buf_len);
    HAL_ADC_Start_DMA(&hadc5, (uint32_t *) &v_cap.value_origin, buf_len);
}
