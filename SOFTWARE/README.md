...
l 注意这个工程是有内存分布文件的，因为启动了STM32G474的CCMRAM，将程序存储在这块内存地址上能够加快程序的执行速度。要注意尽量避免将变量存储在CCMRAM内存中，因为这块内存在重定向之前只有内核能够访问，外设都不能访问。附一下内存分布文件，字体使用的Cascadia Code个人很喜欢这个字体。
